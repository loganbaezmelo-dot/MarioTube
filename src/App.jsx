              ...
              <div key={video.id} className="group relative flex flex-col items-center">
                ...
                <div className="relative w-48 md:w-64 aspect-video bg-gray-900 border-4 border-white rounded mb-[-4px] z-10 overflow-hidden group-hover:-translate-y-4 transition-transform duration-300 cursor-pointer shadow-xl">
                  <a href={video.url} target="_blank" rel="noopener noreferrer" className="block w-full h-full">
                     <img 
                      src={`https://img.youtube.com/vi/${video.url.split('v=')[1]?.slice(0,11)}/hqdefault.jpg`}
...
const Header = ({ user, onUploadClick, onMyChannel }) => (
  <header className="sticky top-0 z-50 bg-[#5c94fc] border-b-4 border-black shadow-xl">
    <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
      <div className="flex items-center space-x-3 cursor-pointer" onClick={() => window.location.reload()}>
        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-red-600 rounded-lg border-2 border-black flex items-center justify-center">
      ...
      {user ? (
        <div className="flex items-center space-x-2 sm:space-x-4">
          <button 
            onClick={onMyChannel}
            className="hidden md:flex items-center space-x-2 bg-black/20 px-3 py-1 rounded-full hover:bg-black/40 transition-colors"
          >
             {user.photoURL ? (
                <img src={user.photoURL} alt="User" className="w-4 h-4 rounded-full border border-white" />
             ) : (
