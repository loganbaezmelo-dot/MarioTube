...
const WatchScreen = ({ video, currentUser, onBack, onNavigateToChannel, subscribedTo = [] }) => {
  ...
  const [isPlaying, setIsPlaying] = useState(false); // Auto-play usually blocked, so we start paused or muted usually, but let's try auto
  ...
  const currentLikes = likedBy.length;
  ...
  const handleShare = () => {
    const shareUrl = window.location.href; // In a real app this would be specific
    navigator.clipboard.writeText(`Check out "${video.title}" on MarioTube!`);
    setShowShare(true);
        ...
        <div className="bg-[#333] border-4 border-black p-6 rounded-lg shadow-lg flex flex-col md:flex-row gap-6 justify-between">
          ...
          <div className="flex md:flex-col gap-3 justify-start md:items-end">
             <button 
               ...
               className={`flex items-center gap-2 px-6 py-3 rounded-full border-2 transition-all active:scale-95 ${hasLiked ? 'bg-white text-red-600 border-red-600' : 'bg-[#444] border-gray-600 hover:bg-[#555]'}`}
             >
               <Heart size={20} fill={hasLiked ? "currentColor" : "none"} />
               <span className="font-bold pixel-font">{currentLikes} LIKES</span>
             </button>
...
const WarpZone = ({ targetUser, videos, currentUser, onBack, subscribedTo = [] }) => {
          ...
          {userVideos.length === 0 ? (
            ...
            userVideos.map((video, index) => (
               <div key={video.id || index} className="group relative flex flex-col items-center">
                  ...
                  <div className="relative w-48 md:w-64 aspect-video bg-gray-900 border-4 border-white rounded mb-[-4px] z-10 overflow-hidden shadow-xl">
                    <img 
...
// --- VIDEO CARD COMPONENT (FEED PREVIEW) ---
  ...
  return (
    ...
    >
      <div className="relative aspect-video bg-black group">
          {/* Static Thumbnail + Hover Play Button */}
...
export default function App() {
  ...
  // Navigation State
  const [currentView, setCurrentView] = useState('feed'); // 'feed' | 'channel' | 'watch'
  const [channelTarget, setChannelTarget] = useState(null);
  const [watchTarget, setWatchTarget] = useState(null); // New state for video being watched
