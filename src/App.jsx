...
const WarpZone = ({ targetUser, videos, currentUser, onBack }) => {
  ...
  const getThumbnail = (video) => {
    if (video.url.includes('firebasestorage')) {
      return null; // Will render placeholder in VideoCard logic
    }
...
// --- VIDEO CARD COMPONENT (HYBRID) ---
const VideoCard = ({ video, user, onUserClick }) => {
  const [isPlaying, setIsPlaying] = useState(false);
  const videoRef = useRef(null);

  // Helper to detect if it's a file or youtube
      ...
      <div className="relative aspect-video bg-black group">
        {isPlaying ? (
          isUploadedFile ? (
            ...
            <iframe
              src={getYoutubeId(video.url) ? `https://www.youtube.com/embed/${getYoutubeId(video.url)}?autoplay=1` : video.url}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (activeTab === 'file') {
        ...
        return; // Wait for upload callback
      } else {
         if (!url) return;
         await createVideoDoc(finalUrl);
      }
