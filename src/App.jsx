            ...
            userVideos.map((video, index) => (
              <div key={video.id} className="group relative flex flex-col items-center">
                ...
                <div className="relative w-48 md:w-64 aspect-video bg-gray-900 border-4 border-white rounded mb-[-4px] z-10 overflow-hidden group-hover:-translate-y-4 transition-transform duration-300 cursor-pointer shadow-xl">
                   {/* Standard Video Tag Placeholder */}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-black/80 p-2 transform translate-y-full group-hover:translate-y-0 transition-transform">
                    <p className="text-[10px] text-white pixel-font truncate">{video.title}</p>
                  </div>
                </div>
    ...
    <div className="bg-white border-4 border-black rounded-xl overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,0.2)] transform transition-all hover:-translate-y-1">
      <div className="relative aspect-video bg-black group">
        {isPlaying ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center cursor-pointer relative bg-[#9290ff]" onClick={() => setIsPlaying(true)}>
             {/* Mario Sky Background Placeholder since we have no thumbnail */}
             <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/brick-wall.png')]"></div>
             
             {/* Cloud decoration */}
             <div className="absolute top-4 right-4 text-white opacity-50"><Upload size={48} className="rotate-90" /></div>
             ...
             <span className="absolute bottom-4 text-white font-black pixel-font text-xs uppercase drop-shadow-md">Click to Start Level</span>
          </div>
        )}
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            ...
            <input 
              ...
              placeholder="Ex: My First Speedrun"
