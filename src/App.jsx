...
// --- VIDEO CARD COMPONENT (MKTV STYLE + WIPED LIKES) ---
const VideoCard = ({ video, user, onUserClick }) => {
  ...
  // --- LIKE LOGIC ---
  // If 'likedBy' doesn't exist (old data), assume empty array (Wipe effect)
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      if (hasLiked) {
        ...
        // Like (Handles missing array by creating it via arrayUnion if field missing)
        await updateDoc(docRef, { 
          likedBy: arrayUnion(user.uid)
        });
      }
    } catch (error) { 
      // If the document is extremely old and missing fields, we might need setDoc merge, but arrayUnion usually handles it.
  ...
  return (
    ...
    >
      <div className="relative aspect-video bg-black group">
        ...
        {/* MKTV Watermark Layer - Visible on ALL videos when playing */}
        {isPlaying && (
        ...
        {isPlaying ? (
          isUploadedFile ? (
            <>
              ...
              {/* MKTV Custom Controls (Only for Files) */}
              <div 
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <button 
            ...
            className={`
              ...
              ${
                hasLiked 
                  ? 'bg-red-500 border-black text-white' // Active State
                  : user 
                    ? 'bg-yellow-400 border-black text-black hover:bg-yellow-300' // Inactive but logged in
                    : 'bg-gray-100 border-gray-300 opacity-50 cursor-not-allowed' // Logged out
              }
            `}
          >
...
const UploadModal = ({ isOpen, onClose, user }) => {
  ...
  const createVideoDoc = async (videoUrl) => {
    await addDoc(collection(db, VIDEO_COLLECTION), {
      ...
      url: videoUrl,
      likes: 0,
      likedBy: [], // Start with empty array for new videos
      userId: user.uid,
